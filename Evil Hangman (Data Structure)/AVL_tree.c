...
TREE avl_tree_init_default( void ) // Making Space for the tree
{
...
// Pushing back
{
    ...
    if ( pMy_tree -> root == NULL ) {
        ...
        if ( pMy_tree -> root == NULL )
            return FAILURE ;
        // Allocating space for the root and making both children = null if there isn't any
        ( pMy_tree -> root ) -> left = NULL;
        ( pMy_tree -> root ) -> right = NULL;
        ( pMy_tree -> root ) -> word_key = my_string_init_c_string( my_string_c_str( key ) ) ;
        ...
        val = my_string_compare( ( pMy_tree -> root ) -> word_key, key ) ;
...
// If the root is not NULL then I destroy both children first and then the parent
    ...
    if ( pMy_tree -> root != NULL) {
        ...
        my_string_destroy( & ( ( pMy_tree -> root ) -> word_key ) ) ;
...
void avl_tree_destroy_node( Node *pNode )
{
    ...
    if ( pNode -> word_key != NULL )
        my_string_destroy( &( pNode -> word_key ) ) ;
...
GENERIC_VECTOR find_max_family( Node *root, int print_val ) {
    ...
    if ( print_val ) {
        ...
        my_string_insertion( root -> word_key, stdout ) ;
